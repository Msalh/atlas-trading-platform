...
from atlas.api.v1 import dashboard, webhook
...
def test_normal_webhook_stores_forwards_once_and_analyzes(client, get_trade):
    ...
    with patch.object(webhook, "forward_to_pickmytrade", return_value=(True, 200, None)) as mock_forward, \
         patch.object(webhook, "analyze_with_claude", return_value=("Looks aligned with trend.", None)) as mock_claude:
...
def test_duplicate_webhook_does_not_forward_twice(client, get_trade):
    ...
    with patch.object(webhook, "forward_to_pickmytrade", return_value=(True, 200, None)), \
         patch.object(webhook, "analyze_with_claude", return_value=("first analysis", None)):
    ...
    with patch.object(webhook, "forward_to_pickmytrade", return_value=(True, 200, None)) as mock_forward_2, \
         patch.object(webhook, "analyze_with_claude", return_value=("should not run", None)) as mock_claude_2:
...
def test_claude_failure_does_not_block_or_prevent_pickmytrade_forward(client, get_trade):
    ...
    with patch.object(webhook, "forward_to_pickmytrade", return_value=(True, 200, None)) as mock_forward, \
         patch.object(webhook, "analyze_with_claude", side_effect=claude_raises):
    ...
    assert trade["pmt_forwarded"] is True  # order relay succeeded regardless of Claude
...
def test_pickmytrade_failure_is_recorded_and_visible(client, get_trade):
    ...
    with patch.object(webhook, "forward_to_pickmytrade",
         ...
         patch.object(webhook, "analyze_with_claude", return_value=("analysis ran fine", None)):
    ...
    html = dashboard.render_trade(trade)

...
**Ground truth this plan is based on**: as of this writing there is **no live deployment** — `docs/staging/deployment-checklist.md` documents an intended Railway-only target (backend and frontend as two separate services in one Railway project, alongside a Railway Postgres plugin), but no git remote is configured and no platform account access exists yet.

---

## Execution order

- **Stage 0**: establish git remote/platform access, choose deployment roots/build commands, document exact Railway configuration for both services. **Requires the user** — account creation and git remote configuration are not actions available to be taken unilaterally.
...
The second hop is a server-to-server HTTP request issued from the frontend service's own Next.js server process, not a request the *browser* makes. Browser CORS governs only requests a browser issues directly to a cross-origin server — it has no jurisdiction over one Railway service calling another, exactly as `fetch()` from any backend-to-backend caller is never subject to CORS, regardless of whether both services happen to run on the same platform or different ones. Concretely: even with `FRONTEND_ORIGINS` left at its `http://localhost:3000` default, the deployed BFF can still reach the deployed Atlas API just fine, because the browser never talks to Atlas directly for any UI v2 page. (Deployment architecture: Railway-only — the frontend service and backend service are two services in the same Railway project, not two different platforms; see `docs/ui_v2/deployment-runbook.md`.)

`FRONTEND_ORIGINS` is real and still required, but only for the **legacy/direct-browser call sites** that predate the BFF and remain in use: `src/lib/api.ts` (trades/analytics/risk/etc. pages) and `src/lib/ruleEngineApi.ts`'s manual-key path (the pre-UI-v2 `/rule-engine` page). Each of those issues a real cross-origin browser request to the Railway origin and will be rejected by CORS if `FRONTEND_ORIGINS` doesn't list the frontend service's public Railway domain. If any of those pages remain in use in production, `FRONTEND_ORIGINS` must be set correctly for *them* — but that is a distinct requirement from "the BFF can reach Atlas," which needs no CORS configuration at all.

**Production hardening, post-Stage-2: `src/lib/live-updates.tsx`'s SSE connection moved out of this list.** It previously opened `EventSource` directly, cross-origin, to `/api/v1/stream?api_key=${NEXT_PUBLIC_API_KEY}` — a real, observed leak (the shared API key appeared verbatim in Railway's own access logs on every page load, across the whole app, not just legacy pages). It now connects same-origin to this app's own `app/api/stream/route.ts`, which proxies to the backend server-side using `ATLAS_API_KEY` as a normal `Authorization` header — the same trust boundary as the JSON BFF, not subject to CORS, and with no key of any kind ever reaching the browser or a URL. The backend's `require_api_key_for_stream` query-parameter fallback has been deleted entirely (not deprecated, not kept for compatibility) — `/api/v1/stream` now uses the same `require_api_key` dependency as every other protected route, and `?api_key=...` alone, even with the correct key, is rejected with a `401`.

**Frontend (Railway target — a second service in the same project as the backend)**
- `NEXT_PUBLIC_API_BASE_URL` — used by both the JSON BFF proxy route and the SSE proxy route (`app/api/stream/route.ts`, server-side, not subject to CORS as above) to reach the Atlas API; also still read by the legacy direct-fetch code paths listed above, where it **is** subject to CORS since those requests originate in the browser. Because this one variable is read both server-side and client-side, it must be set to the backend service's **public** Railway URL, never an internal `*.railway.internal` one — a browser cannot resolve that. See `docs/ui_v2/deployment-runbook.md` §4 for the full internal-vs-public distinction and why a private-networking optimization for the BFF/SSE proxies would require a small follow-up code change, not implemented today.
- `ATLAS_API_KEY` — server-only, must exactly match the backend's `API_KEY`. **Validation task**: confirm this is set in the frontend Railway service's server-side env (not the `NEXT_PUBLIC_` bucket) and is absent from the client bundle (Section 5 covers the actual grep check).
...
Validate the deployed frontend service can actually reach the deployed backend service: DNS resolves, TLS is valid, the public request path the proxy uses today is reachable (both services are in the same Railway project, but the BFF's own `fetch()` still goes over the public URL today, per `NEXT_PUBLIC_API_BASE_URL`'s dual server/browser role above — confirm this is public-internet reachable, not blocked). Confirm the proxy's `AbortSignal.timeout(10_000)` is a sane bound for real network latency between the two services (not just local-loopback latency, which is what all testing to date has used). Once a dedicated server-only internal-URL variable exists (a documented future optimization, not implemented today), this same validation should be repeated against Railway's private network path specifically.
...
- **`ATLAS_API_KEY` absence, four surfaces**:
  1. Client assets — `grep -r "ATLAS_API_KEY" frontend/.next/static/` (already confirmed clean in this engagement's own build; re-confirm against the actual production build artifact the frontend Railway service deploys, not just a local build).
  ...
  3. Logs — check both Railway services' (backend and frontend) actual log output for the key ever appearing (e.g. in a stack trace, a debug log line, a framework's own request logging).
